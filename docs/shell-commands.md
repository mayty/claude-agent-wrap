...
- **`-j`/`--json`** — emits the whole report as one JSON document instead of tables, for scripting and fleet monitoring. Only secret *names* ever appear — less than [`agent secrets check`](#agent-secrets) reports, which also gives each present secret's length and a masked hint; no secret value, master key, or upstream token is ever read into the report.
