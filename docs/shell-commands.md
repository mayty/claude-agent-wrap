...
Rebuilds the resolved image with `--no-cache`, passing `HOST_UID`/`HOST_GID` build args. This is the *force*: [`agent run`](#agent-run) already builds what is missing or stale by itself, so what is left for this verb is the rebuild the wrapper cannot infer — most often applying an edit you just made to `.claude-agent-wrap/Dockerfile`. The base image is still ensured underneath, so a project build never runs on an absent or stale base.
