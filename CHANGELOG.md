...
secret that you pasted twice, `agent secrets check` reports each secret's length and a
masked hint, and `agent secrets` prints its results as a table. A `startup.sh` can now
