...
_USAGE_TEXT = (
    "Usage: agent stats [-v|--verbose] [-p|--pattern P] [-f|--from D] [-u|--until D] "
    "[-d|--days N] <projects.txt>\n\n"
    ...
    "Project filtering:\n"
    "  -p, --pattern P  regex pattern to filter projects by display name\n"
    '                    (e.g. "api", "^(proj-a|proj-b)$", "my-proj")\n\n'
...
_USAGE_LINE = (
    "Usage: agent stats [-v|--verbose] [-p|--pattern P] "
    "[-f|--from D] [-u|--until D] [-d|--days N] <projects.txt>"
)
...
def _show_orphaned(parsed: UsageArgs) -> bool:
    """Return True when the orphaned row should be included for *parsed* pattern."""
    return parsed.pattern is None or parsed.pattern.search(ORPHANED_LABEL) is not None
...
def _filter_projects(
    ...
    """Return *projects* filtered by *parsed.pattern*, or None if none match."""
    if parsed.pattern is not None:
        projects = [p for p in projects if parsed.pattern.search(str(p))]
        if not projects:
            dsp.error(f"No projects match pattern '{parsed.pattern.pattern}'.")
            return None
    return projects
...
def run(args: list[str]) -> int:
    ...
    # Keep the full list for orphaned-dir detection; filtered list for display.
    all_projects = projects
    projects = _filter_projects(projects, parsed, dsp)
    if projects is None:
        return 0

    # Only scan and aggregate orphaned spend when the pattern allows it.
    orphaned_dirs: list[Path] = []
    if _show_orphaned(parsed):
        orphaned_dirs = stats.orphaned_log_dirs(all_projects)
