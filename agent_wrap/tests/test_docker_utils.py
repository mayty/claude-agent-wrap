...
"""Tests for agent_wrap/docker_utils.py."""
...
from agent_wrap.docker_utils import get_user_args, image_exists, is_rootless

# --- is_rootless ---


def test_rootless_true(mocker: pytest_mock.MockFixture) -> None:
    mock_run = mocker.patch("agent_wrap.docker_utils.subprocess.run")
...
def test_user_args_empty_when_rootless(mocker: pytest_mock.MockFixture) -> None:
    mocker.patch("agent_wrap.docker_utils.is_rootless", return_value=True)
    assert get_user_args() == []


def test_user_args_returns_uid_gid_when_not_rootless(mocker: pytest_mock.MockFixture) -> None:
    mocker.patch("agent_wrap.docker_utils.is_rootless", return_value=False)
