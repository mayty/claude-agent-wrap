...
- **Network membership**: a sidecar joins `agent-wrap-net` and nothing else. When a project supplies its own network (`--network X` in `agent-run-args`), it is the *agent* that joins both — the runner emits the `--network agent-wrap-net` flag itself, so an agent container (`--rm`) releases both endpoints when it exits, and a shared sidecar never holds a project's network open.
