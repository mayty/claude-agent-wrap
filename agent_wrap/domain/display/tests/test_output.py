...
def test_error_is_tagged_and_red_on_a_tty(
    capsys: pytest.CaptureFixture[str], tty_display: DisplayService
) -> None:
    tty_display.error(MESSAGE)
...
def test_error_keeps_its_tag_off_a_tty(
    capsys: pytest.CaptureFixture[str], non_tty_display: DisplayService
) -> None:
    non_tty_display.error(MESSAGE)
...
def test_banner_is_marked_and_purple_on_a_tty(
    ...
    tty_display.banner(BANNER_TEXT)
...
def test_banner_keeps_its_marker_off_a_tty(
    ...
    non_tty_display.banner(BANNER_TEXT)
...
def test_continuation_lines_align_under_the_error_tag(
    ...
    non_tty_display.error("first thing went wrong\nthen do this\nand this")
...
def test_continuation_lines_align_under_the_wider_warning_tag(
    ...
    non_tty_display.warning("heads up\ndetail")
