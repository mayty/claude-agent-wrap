...
#
# Provider plugin: LiteLLM-DashScope sidecar.
#
# This file is the entry point for the `litellm-dashscope` provider — selected
# by the launcher when `AGENT_PROVIDER=litellm-dashscope`. It owns
# the lifecycle of a shared LiteLLM container that fronts Alibaba Cloud
# DashScope for every agent on the host.
#
...
#                                   and the network/--add-host flags needed to
#                                   reach the sidecar from the agent's network
#                                   namespace.
...
#
#       The DashScope key is read inline from
#       ${HOME}/claude_keys.json (.DashScopeAPIKey).
...
__litellm_lock_file()    { printf '%s/.agent-launches/litellm-dashscope.lock\n' "$1"; }
__litellm_refcount_file(){ printf '%s/.agent-launches/litellm-dashscope.refcount\n' "$1"; }
...
_provider_ensure() {
    ...
    if __litellm_is_running; then
        ...
        # Cold start: read the DashScope key. Skipped on the reuse path —
        # the running sidecar already holds it and a missing/invalid secrets
        # file shouldn't block agents that don't need it.
        local DASHSCOPE_KEY
        if ! DASHSCOPE_KEY=$(__litellm_read_dashscope_key); then
            exec 9>&-; return 1
        fi
        MASTER_KEY="$(__litellm_generate_master_key)" || {
            exec 9>&-; return 1; }
        if ! __litellm_start "$DASHSCOPE_KEY" "$MASTER_KEY" "$SIDECAR_MODE"; then
            exec 9>&-; return 1
        fi
    ...
    local BASE_URL="http://${_LITELLM_CONTAINER}:${_LITELLM_INTERNAL_PORT}"
    ...
    local AGENT_ENV_ARGS=(
        -e "ANTHROPIC_API_KEY=${MASTER_KEY}"
...
__litellm_read_dashscope_key() {
    ...
    local KEY
    KEY=$(jq -r '.DashScopeAPIKey // empty' "$SECRETS_FILE")
    if [ -z "$KEY" ]; then
        echo "litellm-sidecar: .DashScopeAPIKey missing or empty in $SECRETS_FILE" >&2
        return 1
    fi
    printf '%s' "$KEY"
}
...
#
# The full env dump (including the user's real DASHSCOPE_API_KEY) is
...
__litellm_start() {
    local DASHSCOPE_KEY="$1" MASTER_KEY="$2" SIDECAR_MODE="${3:-bridge}"
    local CONFIG_FILE
    ...
    if [ "$SIDECAR_MODE" = "host" ]; then
        ...
        # DashScope traffic also escapes the bridge / FORWARD chain. Caveat:
    ...
    #
    # DASHSCOPE_API_BASE is the upstream endpoint. The host's environment does
    # NOT repoint it; to target a different endpoint, fork this line together
    ...
    docker run -d --rm \
        ...
        -e DASHSCOPE_API_KEY="$DASHSCOPE_KEY" \
...
__litellm_reconcile_refcount() {
    ...
    while IFS= read -r id; do
        [ -z "$id" ] && continue
        if printf '%s\n' "$LIVE" | grep -Fxq "$id" 2>/dev/null; then
            printf '%s\n' "$id" >> "$TMP"
        fi
    done < "$F"
    mv "$TMP" "$F" 2>/dev/null || rm -f "$TMP"
